...
class ParoleNet(torch.nn.Module):
    ...
    def __init__(
        ...
        self.logger = cu.create_logger(f"{config['name']}",  self.model_dir + "/logs.txt")
    ...
    def forward(self, input_data: torch.Tensor) -> torch.Tensor:
        ...
        input_data["audio"] = input_data["audio"].to(DEVICE)
        input_data["text"] = input_data["text"].to(DEVICE)
        input_data["label"] = input_data["label"].to(DEVICE)
        ...
        wave2vec_output = torch.flatten(wave2vec_output, start_dim=1)
        bert_output = torch.flatten(bert_output, start_dim=1)

        combined_output = torch.cat((wave2vec_output, bert_output), dim=1)
    ...
    def evaluate(self, dataloader: DataLoader, turn_after: float) -> float:
        ...
        self.logger.info(f"Score : {(f1_0*turn_after+ f1_1*(1-turn_after))}")

    def train_one_epoch(self, dataloader: DataLoader, optimizer, turn_after: float) -> float:
        ...
        try:
            for _, batch in enumerate(tqdm(dataloader, desc="Training")):
                try:
                    ...
                    epsilon = torch.tensor([1e-10]).to(DEVICE)  # 1e-10
                    ...
                    loss = -torch.log(
                        (f1_0 * turn_after + f1_1 * (1 - turn_after)) + epsilon
                    )
    ...
    def train_loop(self, data: pd.core.frame.DataFrame, audio_path: str):
        ...
        optimizer = torch.optim.Adam(model.parameters(), lr=self.config["training"]["scheduler"]["initial_lr"])
        scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=self.config["training"]["scheduler"]["step_size"], gamma=self.config["training"]["scheduler"]["gamma"])
